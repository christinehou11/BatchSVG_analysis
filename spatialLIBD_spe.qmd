...
Using `nSD_svg()` function, we determined that `nSD_dev = 4` and `nSD_rank = 6`. Then, we applied `biasDetect()` function on `spe` object to identify and filter out biased genes. We used both thresholds (`threshold = "both"`) and customized the plot color (`plot_palette = "RdPu"`), the point shape and shape (`plot_point_shape = 23`), and the text size for biased genes (`plot_text_size = 4`).
...
```{r subject-related plots, fig.height=6, fig.width=14}
